...
MySQL.prototype.buildSelect = function(model, filter, options) {
  ...
  var selectStmtStr = 'SELECT ' +
    this.buildColumnNames(model, filter) +
    ' FROM ' + this.tableEscaped(model);

  if (options && options.indexHint && typeof options.indexHint === 'string') {
    selectStmtStr += ' USE INDEX(' + this.escapeName(options.indexHint) + ')';
  }

  var selectStmt = new ParameterizedSQL(selectStmtStr);
